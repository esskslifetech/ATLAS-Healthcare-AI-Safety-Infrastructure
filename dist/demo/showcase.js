...
exports.createShowcaseSnapshot = createShowcaseSnapshot;
exports.renderShowcase = renderShowcase;
...
function countTreeNodes(nodes, predicate) {
    let count = 0;
...
function getReadinessVerdict(score) {
    if (score >= 90)
        return 'EXCELLENT';
    if (score >= 80)
        return 'STRONG';
...
function getDeploymentVerdict(overallProgressPercent, readinessScore) {
...
function computeSummary(model) {
    ...
    const completedComplianceControls = model.complianceFeatures.filter((feature) => feature.status === 'COMPLETE').length;
    ...
    const overallProgressPercent = totalPlanned === 0 ? 0 : Math.floor((totalBuilt / totalPlanned) * 100);
    const weightedTotal = sum(model.readinessAssessment.map((metric) => metric.score * (metric.weight ?? 1)));
    const weightSum = sum(model.readinessAssessment.map((metric) => metric.weight ?? 1));
    const readinessScore = weightSum === 0 ? 0 : Math.floor(weightedTotal / weightSum);
...
function validateModel(model) {
    ...
    for (const metric of model.implementationStatus) {
        ...
        if (!Number.isInteger(metric.planned) || metric.planned < 0) {
            throw new ShowcaseValidationError(`implementationStatus.planned must be a non-negative integer for "${metric.component}"`);
        }
        if (!Number.isInteger(metric.built) || metric.built < 0) {
            throw new ShowcaseValidationError(`implementationStatus.built must be a non-negative integer for "${metric.component}"`);
        }
        if (metric.built > metric.planned) {
            throw new ShowcaseValidationError(`implementationStatus.built cannot exceed planned for "${metric.component}"`);
        }
    }
    for (const readiness of model.readinessAssessment) {
        ...
        if (!Number.isFinite(readiness.score) || readiness.score < 0 || readiness.score > 100) {
            throw new ShowcaseValidationError(`readinessAssessment.score must be between 0 and 100 for "${readiness.aspect}"`);
        }
        if (readiness.weight != null &&
            (!Number.isFinite(readiness.weight) || readiness.weight <= 0)) {
            throw new ShowcaseValidationError(`readinessAssessment.weight must be greater than 0 for "${readiness.aspect}"`);
        }
    }
...
function createShowcaseSnapshot(model = DEFAULT_MODEL) {
    validateModel(model);
    return {
...
function renderTreeNodeLabel(node) {
...
function renderBullets(values, bullet = '•') {
    return values.map((value) => `${bullet} ${value}`);
}
function renderBox(title, contentLines, width = 74) {
...
function renderTable(headers, rows) {
    ...
    const makeBorder = (left, middle, right, fill) => `${left}${widths.map((width) => fill.repeat(width + 2)).join(middle)}${right}`;
    const renderRow = (row) => `│ ${row.map((cell, index) => padEndSafe(cell ?? '', widths[index])).join(' │ ')} │`;
    return [
        makeBorder('┌', '┬', '┐', '─'),
        renderRow(headers),
        makeBorder('├', '┼', '┤', '─'),
...
function renderArchitectureText(model) {
    const lines = ['PROMPT OPINION PLATFORM', ''];
    model.architecture.forEach((layer, layerIndex) => {
        lines.push(layer.title);
        for (const capability of layer.capabilities) {
            lines.push(`  ${statusIcon(capability.status)} ${capability.name}${capability.note != null ? ` (${capability.note})` : ''}`);
        }
...
function renderImplementationRows(metrics) {
    ...
    const totalPercent = totalPlanned === '0'
        ? '0%'
        : `${Math.floor((Number(totalBuilt) / Number(totalPlanned)) * 100)}%`;
    return [
        ...rows,
        ['OVERALL PROGRESS', totalPlanned, totalBuilt, `✅ ${totalPercent}`],
    ];
}
function renderReadinessRows(readinessMetrics) {
...
function renderText(snapshot) {
    ...
    sections.push(...model.complianceFeatures.map((feature) => `${statusIcon(feature.status)} ${feature.control}: ${feature.description}`));
    sections.push(`\n🎭 ${model.demoScenarioName.toUpperCase()} DEMO SCENARIO:`);
    sections.push(...model.demoSteps.map((step) => `Step ${step.stepNumber}: ${statusIcon(step.status)} ${step.title}`));
    sections.push('\n⚙️ TECHNICAL SPECIFICATIONS:');
    sections.push(...model.technicalSpecifications.map((specification) => `✅ ${specification.name}: ${specification.value}`));
    sections.push('\n🏆 COMPETITIVE ADVANTAGES:');
    sections.push(...model.competitiveAdvantages.map((advantage) => `${statusIcon(advantage.status)} ${advantage.title}: ${advantage.description}`));
    sections.push('\n📊 IMPLEMENTATION STATUS:');
    sections.push(...renderTable(['Component', 'Planned', 'Built', 'Status'], renderImplementationRows(model.implementationStatus)));
    sections.push('\n🚀 READINESS ASSESSMENT:');
    sections.push(`Overall Score: ${computed.readinessScore}/100 - ${computed.readinessVerdict}`);
    sections.push('');
    sections.push(...renderTable(['Aspect', 'Score', 'Notes'], renderReadinessRows(model.readinessAssessment)));
    sections.push('');
    sections.push(...computed.badges);
    sections.push('\n🎯 NEXT STEPS:');
    sections.push(...model.nextSteps.map((step, index) => `${index + 1}. ${step}`));
    sections.push('\n📌 COMPUTED SUMMARY:');
    sections.push(`• Generated At: ${snapshot.generatedAt}`);
    sections.push(`• Version: ${APP_VERSION}`);
    sections.push(`• Packages: ${computed.completedPackages}/${computed.totalPackages} complete`);
    sections.push(`• Compliance Controls: ${computed.completedComplianceControls}/${computed.totalComplianceControls} complete`);
    sections.push(`• Demo Coverage: ${computed.completedDemoSteps}/${computed.totalDemoSteps} steps complete`);
...
function renderMarkdown(snapshot) {
    ...
    for (const layer of model.architecture) {
        ...
        markdown.push('');
        markdown.push(...layer.capabilities.map((capability) => `- ${statusIcon(capability.status)} ${capability.name}${capability.note != null ? ` (${capability.note})` : ''}`));
        markdown.push('');
    }
    markdown.push('## HIPAA Compliance Features');
    markdown.push('');
    markdown.push(...model.complianceFeatures.map((feature) => `- ${statusIcon(feature.status)} **${feature.control}:** ${feature.description}`));
    markdown.push('');
    markdown.push(`## ${model.demoScenarioName} Demo Scenario`);
    markdown.push('');
    markdown.push(...model.demoSteps.map((step) => `- Step ${step.stepNumber}: ${statusIcon(step.status)} ${step.title}`));
    markdown.push('');
    ...
    markdown.push('');
    markdown.push(...model.competitiveAdvantages.map((advantage) => `- ${statusIcon(advantage.status)} **${advantage.title}:** ${advantage.description}`));
    markdown.push('');
    markdown.push('## Implementation Status');
    markdown.push('');
    markdown.push('| Component | Planned | Built | Status |');
    markdown.push('|---|---:|---:|---|');
    markdown.push(...renderImplementationRows(model.implementationStatus).map((row) => `| ${row[0]} | ${row[1]} | ${row[2]} | ${row[3]} |`));
    markdown.push('');
    markdown.push('## Readiness Assessment');
    markdown.push('');
    markdown.push('| Aspect | Score | Notes |');
    markdown.push('|---|---:|---|');
    markdown.push(...renderReadinessRows(model.readinessAssessment).map((row) => `| ${row[0]} | ${row[1]} | ${row[2]} |`));
    markdown.push('');
...
function renderShowcase(snapshot, format = 'text') {
    switch (format) {
        case 'text':
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (argument === '--format') {
            ...
            if (nextValue !== 'text' && nextValue !== 'json' && nextValue !== 'markdown') {
                throw new ShowcaseValidationError('Invalid value for --format. Expected one of: text, json, markdown');
            }
            format = nextValue;
            index += 1;
            continue;
