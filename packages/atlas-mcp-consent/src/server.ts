...
export const ConsentRequestSchema = z.object({
  patientId: z.string(),
  consentType: z.enum(['TREATMENT', 'RESEARCH', 'DATA_SHARING', 'EMERGENCY']),
  scope: z.array(z.string()),
  expiration: z.string().optional(), // ISO datetime
  providerId: z.string(),
  purpose: z.string(),
});
...
class ConsentManager {
  ...
  async checkConsent(check: ConsentCheck): Promise<{ allowed: boolean; consentId?: string; reason?: string }> {
    ...
    const actionInScope = validConsent.scope.some(scope => 
      check.action.toLowerCase().includes(scope.toLowerCase()) ||
  ...
  getConsentHistory(patientId: string): ConsentRecord[] {
  ...
  try {
    switch (name) {
      case 'request_consent': {
        const consentRequest = ConsentRequestSchema.parse(args);
      ...
      case 'check_consent': {
        const consentCheck = ConsentCheckSchema.parse(args);
      ...
      case 'revoke_consent': {
        const { consentId } = args as { consentId: string };
