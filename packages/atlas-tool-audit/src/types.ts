...
export const HIPAA_EVENT_CATEGORIES = {
  ACCESS: 'access',
...
export const AuditEventSchema = z.object({
  ...
  event_id: z.string(), // Add event_id for compatibility
  purpose: z.string().optional(), // Add purpose field
  consent_ref: z.string().optional(), // Add consent_ref field
  metadata: z.record(z.any()).optional(), // Add metadata field
});
...
export const AuditQuerySchema = z.object({
  ...
  purpose: z.string().optional(), // Add purpose field
  result: z.enum(['success', 'failure', 'partial']).optional(), // Add result field
  actor_type: z.string().optional(), // Add actor_type field
  resource_type: z.string().optional(), // Add resource_type field
  resource_id: z.string().optional(), // Add resource_id field
});
...
export const AuditStatisticsSchema = z.object({
  ...
  events_by_actor: z.record(z.number()),
  events_by_actor_type: z.record(z.number()).optional(), // Add events_by_actor_type
  events_by_resource_type: z.record(z.number()).optional(), // Add events_by_resource_type
  events_by_result: z.record(z.number()).optional(), // Add events_by_result
  events_by_hour: z.record(z.number()).optional(), // Add events_by_hour
  unique_actors: z.number().optional(), // Add unique_actors field
  date_range: z.object({
    start: z.string(),
    end: z.string(),
  }),
  ...
  error_rate: z.number(),
  access_denied_rate: z.number(),
});

export const ChainValidationSchema = z.object({
  valid: z.boolean(),
  total_events: z.number(),
  first_event_hash: z.string().optional(),
  last_event_hash: z.string().optional(),
  breaks: z.array(z.object({
    event_index: z.number(),
    expected_hash: z.string(),
    actual_hash: z.string(),
  })),
  tampered: z.boolean().optional(), // Add tampered field
  first_tampered_event: z.string().optional(), // Add first_tampered_event field
  tampered_events: z.array(z.string()).optional(), // Add tampered_events field
  chain_integrity: z.string().optional(), // Add chain_integrity field
  validation_timestamp: z.string().optional(), // Add validation_timestamp field
  total_events_checked: z.number().optional(), // Add total_events_checked field
});
...
export const AuditExportSchema = z.object({
  ...
  total_events: z.number().optional(), // Add total_events field
  checksum: z.string(),
  compressed: z.boolean(),
  events: z.array(AuditEventSchema).optional(), // Add events field
  file_size: z.number().optional(), // Add file_size field
  exported_by: z.object({
    id: z.string(),
    type: z.string().optional(), // Add type field
    name: z.string().optional(),
